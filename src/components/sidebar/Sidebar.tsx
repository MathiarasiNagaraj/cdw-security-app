...
/**
 * @description A  sidebar component with  nav tabs
 * @author [Mathiarasi]
 * @returns  function will  sidebar
 */
...
const Sidebar = () => {
  ...
  const routes = SIDEBAR_ROUTES.map((route) => (
    <Link
      href={`${route.url(location)}`}
      key={route.name}
        ...
        <nav className={sidebarClasses.join(" ")}>
          <div className={styles["sidebar-wrapper"]}>
            <div className={styles["text"]}>
              ...
              <div className={styles["nav-item-wrapper"]}>{routes}</div>
            </div>
          </div>
        </nav>
      </div>
